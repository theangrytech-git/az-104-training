...
for_each = { for dept in local.users : dept.department => dept... }

display_name = format("%s-team", lower(each.key),)
...
resource "azurerm_role_assignment" "data_purger" {
  ...
  principal_id         = azuread_user.users[each.key].department == "R&D" == true
}
